...
| Source ingest: Google Drive | critical | present | present | missing | `src/notebooklm/_sources.py` (`add_drive`); CLI `source_add_drive` in `src/notebooklm/cli/source.py`; no corresponding MCP registration in `src/notebooklm_mcp/tools/sources.py` | This is a direct parity gap for BA workflows that depend on Drive docs/slides/sheets as inputs. Add MCP tool and adapter method early. |
| Source list/get/delete/rename metadata management | critical | present | present | partial | `src/notebooklm/_sources.py` (`list`, `get`, `delete`, `rename`); CLI `src/notebooklm/cli/source.py`; MCP has `notebooklm_sources_list` and destructive remove in `src/notebooklm_mcp/tools/sources.py` | MCP lacks a first-class source `get` tool and rename tool. The resource `notebooklm://notebooks/{notebook_id}/sources/{source_id}` is not a full replacement. |
...
| Source refresh | critical | present | present | missing | `src/notebooklm/_sources.py` (`refresh`); CLI `source_refresh` in `src/notebooklm/cli/source.py`; no MCP tool registration for refresh | BA clarification loops need reliable refresh for stale URL/Drive sources. Add MCP parity before BA source curation is designed. |
| Source freshness check | critical | present | present | missing | `src/notebooklm/_sources.py` (`check_freshness`); CLI stale check in `src/notebooklm/cli/source.py`; no MCP tool/resource equivalent | The revised BA plan explicitly wants freshness in snapshots/manifests. This is one of the most important phase-0 gaps. |
| Source guide: summary + keywords | critical | present | present | missing | `src/notebooklm/_sources.py` (`get_guide`); CLI guide command in `src/notebooklm/cli/source.py`; no MCP tool/resource equivalent | BA source triage and context-packing will want guide metadata. Add a dedicated MCP surface, not just fulltext. |
| Source fulltext retrieval | critical | present | present | partial | `src/notebooklm/_sources.py` (`get_fulltext`); CLI fulltext command in `src/notebooklm/cli/source.py`; MCP `notebooklm_sources_get_content` in `src/notebooklm_mcp/tools/sources.py`; source resource in `src/notebooklm_mcp/resources.py` | MCP only exposes capped content preview/fulltext retrieval, not the richer source-intelligence bundle the BA runner wants. Still useful as a base. |
