...
- **Quick reads:** `resource://agents/...`, `resource://inbox/...`, `resource://thread/...`
...
1. **Sync session:**
   ...
   macro_start_session(
     human_key=<abs-path>,
     agent_name="<AdjectiveNoun>",
     program="codex-cli",
     model="gpt-5-codex"
   )
   ```

2. **Check active agents + inbox:**
   ```
   resource://agents/<project-slug>
   resource://inbox/{Agent}?project=<abs-path>&limit=20
   ```

3. **Send a progress update before edits:**
   ```
   send_message(..., thread_id="FEAT-123", subject="[FEAT-123] Start: <scope>")
   ```
...
### Objective
  When a task touches the BA runner, treat `ba.*` as the primary workflow surface. Prefer the workflow-native BA tools over ad hoc NotebookLM calls whenever the goal is to create, inspect, validate, resume, or selectively rerun a
  feature bundle.

  ### Context
  The `ba.*` surface is the distinctive part of this repository. It exists to run evidence-first, deterministic BA implementation-pack workflows with persisted run state and reproducible bundle artifacts.
  ...
  - `<output_dir>/docs/features/<feature_key>/`

### Work Style
  - Work evidence-first. Do not invent product, API, or UI details that are not grounded in sources.
  - Prefer the smallest safe step that preserves deterministic rerun behavior.
  - Go broad first when understanding a feature or bundle layout, then go deep on the failing or changing stage.
  - Re-check non-trivial conclusions with fresh eyes before closing work.

### Tool Rules
  - Default to `ba.run_pipeline` for end-to-end BA execution.
  - Use `ba.start_run` and `ba.register_sources` only when you need staged control or source registration before downstream steps.
  - Use `ba.status` whenever a run halts, degrades, or you need resume guidance.
  - Use `ba.validate_bundle` after manual edits, golden-file changes, or any external mutation to an existing bundle.
  - Use `ba.rerun_impacted` when only part of the source set changed and a full rerun is unnecessary.
  - Use generic `notebooklm_*` tools mainly for notebook provisioning or non-BA tasks outside the workflow-native BA path.
  - Do not rely on preview-capped generic source content helpers as the source of truth for deterministic BA validation decisions.

### Output Contract
  When reporting BA work, include:
  - `feature_key`
  - `run_id` if available
  - the tool(s) used
  - the resolved output directory
  - key warnings, degradations, or resume hints
  - what changed in the bundle
  - what remains to be done, if anything

### Verification
  Before stopping:
  - confirm the selected BA tool matched the task
  - check whether the bundle state is valid, degraded, or resumable
  - call out any assumptions, missing critical sources, or deterministic-repair warnings
  - prefer selective rerun over full rerun when the change scope is narrow
  - mention better alternatives if a chosen path is heavier than necessary

### Done Criteria
  Stop only when one of these is true:
  - the BA run completed successfully and the resulting bundle state is clear
  - the bundle was revalidated and any remaining issues are explicitly documented
  - the next required BA step is identified with a concrete resume path
  - progress is blocked by a specific missing source, external dependency, or upstream failure
